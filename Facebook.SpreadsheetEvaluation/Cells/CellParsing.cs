...
namespace Facebook.SpreadsheetEvaluation.Cells
{
    public static class CellParsing
    {
        public static Cell Parse(string formula)
        {
            ...
            throw new InternalSpreadsheetParserException($"'{formula}' is an invalid content for cell.");
        }
        ...
        private static readonly Regex CellReferenceRegex = new Regex(@"^(?<column>[A-Z]+)(?<row>[1-9]\d*)$", RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.Singleline);
        ...
        private static Operand ParseOperand(string operand)
        {
            switch (operand)
            {
                ...
                default:
                    throw new InternalSpreadsheetParserException($"'{operand}' is not one of the valid operators (+, -, *, /)");
            }
        }
    }
}